...
def signal_receiver() -> bool:
    ...
    Returns True if SIGUSR2 was delivered (pkill exit 0), False if no
    matching process was found or an exception occurred. Callers should
    log a warning on False; config is always committed to DB first so a
    failed signal is not data loss.
    """
    try:
